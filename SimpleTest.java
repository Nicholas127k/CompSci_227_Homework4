...
Point beforeHighPoint = path1.getPointByIndex(highPoint.getPointIndex()
- 1);
...
CouplingLink link2 = new CouplingLink(path1.getHighpoint(),
path2.getLowpoint());
...
// test that the high end of path 1 is connected to the low end of path2
connectedPoint = link2.getConnectedPoint(highPoint);
if (connectedPoint == path2.getLowpoint()) {
System.out.println("Path 1 high point is connected to path 2 lowpoint as expected.");
} else {
System.out.println("Path 1 high point is not connected to path 2low point.");
}
// test shiftPoints
...
StraightLink link3 = new StraightLink(path1.getHighpoint(),
path2.getLowpoint(), path3.getLowpoint());
TurnLink link4 = new TurnLink(path1.getHighpoint(),
path2.getLowpoint(), path3.getLowpoint());
SwitchLink link5 = new SwitchLink(path1.getHighpoint(),
path2.getLowpoint(), path3.getLowpoint());
...
System.out.println("StraightLink: Path 1 high point is notconnected to path 2 low point.");
}
connectedPoint = link4.getConnectedPoint(highPoint);
if (connectedPoint == path3.getLowpoint()) {System.out.println("TurnLink: Path 1 high point is connected to path 3 low point as expected.");
} else {
...
if (connectedPoint == path2.getLowpoint()) {
System.out.println("TurnLink (false): Path 1 high point isconnected to path 2 low point as expected.");
} else {
System.out.println("TurnLink (false): Path 1 high point is notconnected to path 2 low point.");
}
link5.setTurn(true);
connectedPoint = link5.getConnectedPoint(highPoint);
if (connectedPoint == path3.getLowpoint()) {
System.out.println("TurnLink (true): Path 1 high point isconnected to path 3 low point as expected.");
} else {
System.out.println("TurnLink (true): Path 1 high point is notconnected to path 3 low point.");
}
// set up another path to test the multi-path links
Path path4 = track.addPathType(PathTypes.pathType2, 6, 4, NORTH, EAST);
PointPair pair1 = new PointPair(path1.getHighpoint(),
path3.getLowpoint());
PointPair pair2 = new PointPair(path4.getHighpoint(),
path2.getLowpoint());
...
if (connectedPoint == path3.getLowpoint()) {
System.out.println("MultiFixedLink: Path 1 high point isconnected to path 3 low point as expected.");
} else {
System.out.println("MultiFixedLink: Path 1 high point is notconnected to path 3 low point.");
}
link7.switchConnection(new PointPair(path1.getHighpoint(),
path2.getLowpoint()), 0);
connectedPoint = link7.getConnectedPoint(path1.getHighpoint());
if (connectedPoint == path2.getLowpoint()) {
System.out.println("MultiSwitchLink: Path 1 high point isconnected to path 2 low point as expected.");
} else {
System.out.println("MultiSwitchLink: Path 1 high point is notconnected to path 2 low point.");
}
}
}
